...
def load_data(label_list, train_pct=0.8):
    ...
    df = pd.read_excel("/content/drive/My Drive/Colab Notebooks/EyeDetection/Integration/Final-YEET/ODIR-5K_Training_Annotations(Updated)_V2.xlsx")
...
class XceptionSingleClassifier:
    def __init__(self, disease_idx, master_train, master_test, num_epochs=40, base_lr=0.01, lr_decay=0.5,
                 batch_size=36, base_dataset_filepath="/content/drive/My Drive/Colab Notebooks/EyeDetection/Integration/Final-YEET/Images/",
                 working_dataset_filepath="/content/drive/My Drive/Colab Notebooks/EyeDetection/Integration/Final-YEET/New-Images/"):  # change this up here
        """
        Makes the Xception model for 1 class
        ...
        self.logdir = "/content/drive/My Drive/Colab Notebooks/EyeDetection/Integration/Logs_{}".format(disease_idx)
        ...
        # This is how we make the Xception model
        self._model = Xception(input_shape=(299, 299, 3), weights='imagenet', include_top=False)
...
if __name__ == "__main__":
    ...
    demo_model = Xception(include_top=False, classes=95, weights="imagenet")
    # demo_model.summary()
    del demo_model
    K.clear_session()
    gc.collect()
    ...
    for class_idx in range(len(label_list)):
        ...
        model = XceptionSingleClassifier(class_idx, master_train, master_test)
    ...
    sample = XceptionSingleClassifier.load_sample("path/to/image.png")
    class_confidences = []
    for class_idx in range(len(label_list)):
